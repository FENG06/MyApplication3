...
import com.example.zhou.myapplication.*;
...
//        setCompoundDrawablesWithIntrinsicBounds(Drawable left,Drawable top,Drawable right,Drawable botton);
//        作用：在EditText上、下、左、右设置图标（相当于android:drawableLeft=""）
...
//        setClearIconVisible() = 根据传入的但是否选中&是否有输入来判断是否显示删除图标->>关注1
...
//        同样根据setClearIconVisible()判断是否要显示删除图标
    ...
    /**
     * 步骤4：对删除图标区域设置点击时间，即“点击=情况搜索框内容”
     * 原理：当手指抬起的位置再删除图标的区域，即视为点击了删除图标=清空搜索框内容
     */
...
//                当手指抬起的位置在删除图标的区域（X2<=event.getX()<=X1）,即视为点击了删除图标=情况搜索框内容
                break;
        }

...
//         搜索框字体颜色（将用十六进制代码，如#333、#8e8e8e）

        int defaultColor = context.getResources().getColor(R.color.color_c61a04);
...
//        搜索框高度
        searchBlockHeight = typedArray.getInteger(R.styleable.SearchView_searchBlockHeight, 150);

//        搜索框颜色
        searchBlockColor = typedArray.getColor(R.styleable.SearchView_searchBlockColor, getResources().getColor(R.color.color_999999));
...
//                    2.点击搜索键后，对该字段在数据库中是否存在进行检查 ->>关注1

                    boolean hasData = hasData(et_search.getText().toString().trim());

//                    3.若存在，则不保存；若不存在，则将该搜索字段保存到数据库，并作为历史搜索记录
                    if (!hasData) {
...
//        1.绑定R.layout.search_layout为搜索框的xml文件
...
//        6.返回按键
    ...
    /*
    * 关注1
    * 模糊数据查询 & 显示到ListView列表上
    * */

    private void queryData(String tempName) {

//        1.模糊搜索
        Cursor cursor = helper.getReadableDatabase().rawQuery("select id as _id,name from records where name like '%" +
...
//     从数据库Record表里找到name = tempName的id
